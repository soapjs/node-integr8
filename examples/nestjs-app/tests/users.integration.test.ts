...
describe('NestJS Users API Integration Tests', () => {
  ...
  test('should delete a user', async ({ http }) => {
    ...
    // Verify it's deleted
    const getResponse = await http.get(`/users/${userId}`);
    expect(getResponse.status).toBe(500); // NestJS throws error for not found
  });
  ...
  test('should handle validation errors', async ({ http }) => {
    ...
    expect(response.status).toBe(500); // NestJS throws error for validation
  });
...
describe('TypeORM Database State Management', () => {
...
describe('NestJS Override System', () => {
  test('should support service overrides', async ({ http, ctx }) => {
    ...
    await ctx.override.service('UsersService').withMock(() => ({
      findAll: () => Promise.resolve([{ id: 1, name: 'Mock User', email: 'mock@example.com' }])
    }));
  ...
  test('should support repository overrides', async ({ http, ctx }) => {
    // Override a repository (example)
    await ctx.override.repository('UserRepository').withMock(() => ({
      find: () => Promise.resolve([{ id: 1, name: 'Mock User', email: 'mock@example.com' }])
    }));
    
    // Test with overridden repository
    const response = await http.get('/users');
